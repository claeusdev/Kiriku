...
export default class ArticleForm extends Component {
  constructor(props) {
    ...
    const { title, content, coverImageUrl, status } = article;
  ...
  validateForm(articleStatus) {
    const { title, content, coverImageUrl, tags, countries } = this.state;
  ...
  render() {
    ...
    const editArticleSaveMessage = (
      <span>
        {" "}
        <CheckMarkIcon /> Save Changes
      </span>
    );

    const saveMessage =
      mode === "edit" ? editArticleSaveMessage : newArticleSaveMessage;

    const savingMessage = (
      <span>
        {" "}
        One Moment Please <MovingEllipsis />
      </span>
    );
    ...
    return (
      <div className="Apollo-ArticleForm">
        <div className="FormHeader">
          ...
          <Link className="FormBackLink" to={backLinkUrl}>
            <BackIcon /> Back
          </Link>
        </div>
          ...
          <Row className="show-grid">
            <Col xs={12} md={8} className="NewArticleForm-Main">
              ...
              >
                ...
                />
                {titleState === "error" && (
                  <HelpBlock className="ApolloLogin-FormBox-Form-InputError">
                    Please add a title
                  </HelpBlock>
                )}
              </FormGroup>
              ...
              <FormGroup
                className="Form-InputGroup"
                validationState={contentState}
              >
                <ControlLabel>
                  What's an article without some content?
                </ControlLabel>
                <FroalaEditor
                  tag="textarea"
                  model={content}
              ...
              <FormGroup
                className="Form-InputGroup"
                validationState={coverImageUrlState}
              >
                <ControlLabel>Choose a dope cover image</ControlLabel>
                <SingleImagePicker
                  previewUrl={coverImageUrl}
                  onFileUpload={this.handleCoverImageUrlChange}
                  editable={editable}
                />
                {coverImageUrlState === "error" && (
                  <HelpBlock className="ApolloLogin-FormBox-Form-InputError">
            ...
            <Col xs={6} md={4} className="NewArticleForm-Side">
              ...
              <FormGroup
                className="Form-InputGroup"
                validationState={countriesState}
              >
                      ...
                      <div className="PublishSectionContent">
                        <p className="PublishSectionContent-Status">
                          <div className="PublishSectionContent-StatusIcon">
                            {status === "published" && <EyeIcon />}
                            {status === "draft" && <EditIcon />}
                            {status === "unpublished" && <ArchiveIcon />}
                          </div>
                          <span className="PublishSectionContent-StatusLabel">
                            Status:{" "}
                          </span>
                          <span className="PublishSectionContent-StatusValue">{`${status
                            .charAt(0)
                            .toUpperCase()}${status.slice(1)}`}</span>
                          {!showStatusChangeForm && (
                            <a
                              className="PublishSectionContent-StatusChangeLink"
                              href=""
                              onClick={e => {
                                e.preventDefault();
                                this.showStatusChangeForm();
                              }}
                            >
                              Change
                            </a>
                        ...
                        {showStatusChangeForm && (
                          <div className="PublishSectionContent-StatusUpdate">
                            ...
                            >
                              ...
                              <Button onClick={this.hideStatusChangeForm}>
                                Okay
                              </Button>
                              <Button
                                bsStyle="link"
                                onClick={this.discardStatusChanges}
                              >
                                Cancel
                              </Button>
                            </FormGroup>
                ...
                <FormGroup className="Form-InputGroup" validationState={null}>
                  ...
                  <div className="button-group-box">
                    ...
                    >
                      <MenuItem
                        eventKey="1"
                        onSelect={() => this.validateForm("draft")}
                      >
                        <LockIcon /> Save As Draft
                      </MenuItem>
                      <MenuItem
                        eventKey="2"
                        onSelect={() => this.validateForm("published")}
                      >
                        <EyeIcon /> Save and Publish
                      </MenuItem>
                    </DropdownButton>
                  </div>
                </FormGroup>
